...
function VerifyOTP (props) {
  ...
  const navigateToNextPage = (e) => {
    ...
    data.type = "forgot";
    setLoading(true);
    DataService.verifyUserOtp(data).then((data) => {
      setLoading(false);
      location.href = "/reset-password/" + data.data.user.reset_password_token;
          ...
          <form className="mt-3 w-100" onSubmit={navigateToNextPage} ref={form}>
            ...
            <div className="d-flex flex-column gap-3">
              ...
              <p className="text-center m-0">
                <button onClick={resendOtp} className="send-otp">Resend OTP</button>
              </p>
            </div>
          </form>
        </div>

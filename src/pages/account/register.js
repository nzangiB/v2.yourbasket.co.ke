...
async function UserRegistrationForm () {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { user } = await AuthService.register(postData);
      console.log(user);
      if (!user) return;
  ...
  const handleSubmitClick = (e) => {
    e.target.innerHTML = <span>Signing Up ...</span>;
  };

  return (
    <section className={"signup-form"}>
      <div className={"form-container"}>
        {/* <UserRegistrationForm id={"signupForm"} data={data} handleSubmit={handleSubmit}/> */}
        <form id={"userRegistrationForm"} className={"form"} onSubmit={handleSubmit}>
          ...
          <section>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: "10px" }}>
              ...
              <p>I agree with the <a data-route={"/terms-conditions"} className={"link"}>Terms and Conditions</a></p>
            </div>
          </section>
          <footer>
...
function RegistrationFormVerifyOtp (props) {
  ...
  const handleSubmit = async (e) => {
    ...
    const userInfo = localStorage.getItem("user-unregistered") ? JSON.parse(localStorage.getItem("user-unregistered")) : undefined;
    if (!userInfo) {
      toast.error("There's an issue with your registration", { position: toast.POSITION.TOP_RIGHT });
      button.disabled = false;
      return;
    }
    ...
    data.phone = props.phone; // get this from local storage
    data.type = "register";
  ...
  const handleSubmitClick = (e) => {
    e.target.innerHTML = <span>Verifying...</span>;
  };
        ...
        <form className={"form"} onSubmit={handleSubmit}>
          <header>
            ...
            <h4 style={{ textAlign: "center" }}>
							The verification code sent <br/>to ********{props.phone?.slice(-2)}
            </h4>
          </header>
          <section>
            <div className={"input-field"}>
...
function Register (props) {
  ...
  const getPage = (page) => {
    switch (page) {
      case "verify-otp":
        return <RegistrationFormVerifyOtp/>;
      case "vendor":
